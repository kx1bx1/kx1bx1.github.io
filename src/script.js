...
const App = (() => {
  ...
  let extensions = [];
  let favorites = JSON.parse(localStorage.getItem('tw_gallery_favorites') || '[]');
  ...
  const mockExtensions = [
    ...
    },
    {
      id: 'gamepad-pro',
      name: 'Gamepad Pro',
      description: 'Extended support for rumble, gyroscope, and specific controller mappings.',
      author: 'GamerOne',
      version: '1.0.0',
      category: 'hardware',
    ...
    },
    {
      ...
      description: 'Connect to standard WebSocket servers to build multiplayer games or chat apps.',
      author: 'NetMaster',
      version: '0.9.5',
      category: 'network',
      iconValue: 'ph-globe',
    ...
    },
    {
      ...
      description: 'Generate frequency data from audio to create music visualizers and effects.',
      author: 'AudioWiz',
      version: '2.0.1',
      category: 'media',
      ...
      const loaded = await Promise.all(
        folders.map(async (folder) => {
          try {
            ...
            const iconSvgOk = results[1].status === 'fulfilled' && results[1].value.ok;
            const iconTxtOk = results[2].status === 'fulfilled' && results[2].value.ok;

            const category = normalizeCategory(manifest.category);
            const entry = manifest.entry || 'extension.js';
            const url = new URL(`${folder}/${entry}`, window.location.href).href;
            ...
            if (iconSvgOk) {
              iconType = 'svg';
              iconValue = new URL(`${folder}/icon.svg`, window.location.href).href;
            } else if (iconTxtOk) {
              const txt = await results[2].value.text();
              iconType = 'class';
              iconValue = txt.trim().startsWith('ph-') ? txt.trim() : `ph-${txt.trim()}`;
            }
  ...
  function hydrateMockData(data) {
    return data.map((ext) => ({
      ...
      searchString: `${ext.name} ${ext.description} ${ext.author}`.toLowerCase(),
    }));
  }
                    ...
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <div class="flex flex-col items-end gap-2">
                            <button 
                                ...
                                aria-pressed="${isFav}"
                                class="text-xl ${
                                  isFav ? 'text-yellow-400' : 'text-slate-300 dark:text-gray-600'
  ...
  function renderCategories() {
    ...
    els.categoryContainer.appendChild(createBtn('favorites', 'Favorites', 'ph-star-fill'));
  ...
  function toggleFavorite(id, btn) {
    ...
    if (currentCategory === 'favorites') filterAndRender();
      ...
      btn.className = `text-xl transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-yellow-400 rounded-full ${
        isFav ? 'text-yellow-400' : 'text-slate-300 dark:text-gray-600 hover:text-yellow-400'
      }`;
    }
  }
  ...
  function openModal(id) {
    ...
    els.modalDownloadBtn.parentNode.replaceChild(newDownload, els.modalDownloadBtn);
    ...
    els.modalOpenBtn.href = `https://turbowarp.org/editor?extension=${encodeURIComponent(ext.url)}`;
    ...
    els.modalCopyBtn.addEventListener('click', () => copyToClipboard(ext.url, els.modalCopyBtn));
